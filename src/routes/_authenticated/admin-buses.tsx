...
function AdminBuses() {
  ...
  async function save(b: BusRow) {
    ...
    if (b.layout_id) {
      ...
      if (lay) {
        patch.capacity = lay.seat_count || b.capacity;
      }
    }
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        <Input
          className="h-9 w-24"
          placeholder="VIP/عادية"
          value={local.bus_type ?? ""}
          onChange={(e) =>
            setLocal({
              ...local,
        ...
        <Select
          ...
          onValueChange={(v) => {
            ...
            setLocal({
              ...local,
              layout_id: id,
              capacity: lay?.seat_count ?? local.capacity,
            });
          }}
        >
      ...
      <TableCell>
        <span className={free <= 0 ? "text-destructive font-bold" : "font-semibold"}>
          {used}/{local.capacity}
        </span>

        <div className="text-[10px] text-muted-foreground">متبقٍ {free}</div>
      </TableCell>
      ...
      <TableCell className="flex gap-1">
        <Button size="sm" onClick={() => onSave(local)} title="حفظ">
          <Save className="h-3 w-3" />
        </Button>

        <Button size="sm" variant="outline" onClick={onDuplicate} title="نسخ">
          <Copy className="h-3 w-3" />
        </Button>

        <Button size="sm" variant="outline" onClick={onTransfer} title="نقل الحجوزات">
          <ArrowRightLeft className="h-3 w-3" />
        </Button>

        <Button size="sm" variant="outline" onClick={onDelete} title="حذف">
...
function TransferDialog({
  ...
  return (
    <Dialog open={!!from} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>نقل الحجوزات من {from?.name || `حافلة ${from?.bus_number}`}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          <Select value={targetId} onValueChange={setTargetId}>
            <SelectTrigger>
              <SelectValue placeholder="اختر حافلة الهدف" />
            </SelectTrigger>
        ...
        <DialogFooter>
          ...
          <Button disabled={!targetId || busy} onClick={run}>
            {busy ? "جارٍ النقل..." : "نقل الحجوزات"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
