...
interface AdminBooking {
  ...
  trips: { name: string; departure_day: string } | null;
...
function AdminBookings() {
  ...
  const { data: bookings = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("bookings")
        .select("id,booking_code,status,created_at,customer_name,id_number,contact_phone,whatsapp_phone,passenger_count,seat_numbers,total_price,price_per_person,discount_amount,coupon_code,nationality,booking_source,trip_id,bus_id,package_id,no_hotel,no_bus,deleted_at,rep_name,trips(name,departure_day),buses(name,bus_number),packages(name)")
  ...
  const { data: buses = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data ?? []) as Array<{ id: string; name: string | null; bus_number: number }>;
    },
  });

  const filtered = useMemo(() => {
    let list = bookings;
    if (tab === "deleted") list = list.filter((b) => b.deleted_at);
    else list = list.filter((b) => !b.deleted_at);
    ...
    if (tab === "by-bus" && busFilter) list = list.filter((b) => b.bus_id === busFilter);
    const term = q.trim().toLowerCase();
    if (term) {
      list = list.filter((b) =>
        [b.booking_code, b.customer_name, b.contact_phone, b.whatsapp_phone, b.id_number]
  ...
  async function softDelete(b: AdminBooking) {
    if (!confirm(`حذف الحجز ${b.booking_code}؟`)) return;
  ...
  async function restore(b: AdminBooking) {
    const { error } = await supabase.from("bookings").update({ deleted_at: null }).eq("id", b.id);
    if (error) return toast.error(error.message);
  ...
  const busOccupancy = useMemo(() => {
    if (tab !== "by-bus" || !busFilter) return null;
    const rows = bookings.filter((b) => !b.deleted_at && b.bus_id === busFilter);
    const seats = rows.flatMap((b) => b.seat_numbers ?? []);
    return { count: rows.length, seatCount: seats.length, seats };
    ...
    <div className="min-h-screen bg-muted">
      <header className="bg-[color:var(--color-navy)] text-white">
        <div className="container-luxe py-4 flex items-center justify-between">
          <h1 className="text-lg font-extrabold flex items-center gap-2"><Ticket className="h-5 w-5" /> إدارة الحجوزات</h1>
          <Link to="/dashboard">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            {tab === "by-bus" && (
              ...
              >
                ...
                {buses.map((b) => (
                  <option key={b.id} value={b.id}>{b.name ?? `حافلة ${b.bus_number}`}</option>
                ))}
              </select>
            )}
          </div>
          ...
          {tab === "by-bus" && busOccupancy && (
            <div className="mt-3 surface-card p-4 flex flex-wrap gap-6 items-center">
              <div className="flex items-center gap-2 font-bold"><BusIcon className="h-4 w-4" /> حجوزات هذه الحافلة</div>
              <div>عدد الحجوزات: <b>{busOccupancy.count}</b></div>
              <div>عدد المقاعد المشغولة: <b>{busOccupancy.seatCount}</b></div>
            </div>
          )}
                ...
                {filtered.map((b) => (
                  <div key={b.id} className={`surface-card p-4 border ${b.deleted_at ? "opacity-60" : ""}`}>
                    <div className="flex flex-wrap justify-between gap-3">
                      <div className="flex-1 min-w-[240px]">
                        ...
                        <div className="mt-2 text-sm grid grid-cols-1 sm:grid-cols-2 gap-1">
                          <div>العميل: <b>{b.customer_name || "—"}</b></div>
                          <div>الجوال: <b>{b.contact_phone || "—"}</b></div>
                          <div>الفندق: <b>{b.packages?.name || (b.no_hotel ? "بدون" : "—")}</b></div>
                          <div>الحافلة: <b>{b.buses ? (b.buses.name || `حافلة ${b.buses.bus_number}`) : (b.no_bus ? "بدون" : "—")}</b></div>
                          <div>الرحلة: <b>{b.trips?.name || "—"}</b></div>
                          <div>عدد الأفراد: <b>{b.passenger_count}</b></div>
                          {b.seat_numbers?.length ? <div className="col-span-full">المقاعد: <b className="font-mono">{b.seat_numbers.join(", ")}</b></div> : null}
                        </div>
                      </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      {!b.deleted_at && (
                        <>
                          <Button size="sm" variant="outline" className="rounded-xl" onClick={() => editBooking(b.booking_code)}>
                            <Edit className="h-3 w-3 ml-1" /> تعديل
                          </Button>
                          {b.status !== "confirmed" && (
                            <Button size="sm" variant="outline" className="rounded-xl" onClick={() => updateStatus(b, "confirmed")}>تأكيد</Button>
                          )}
                          {b.status !== "cancelled" && (
                            <Button size="sm" variant="outline" className="rounded-xl" onClick={() => updateStatus(b, "cancelled")}>
                              <XCircle className="h-3 w-3 ml-1" /> إلغاء
                            </Button>
                          )}
                          <Button size="sm" variant="outline" className="rounded-xl text-destructive" onClick={() => softDelete(b)}>
                            <Trash2 className="h-3 w-3 ml-1" /> حذف
                          </Button>
                        </>
                      )}
                      {b.deleted_at && (
                        <Button size="sm" variant="outline" className="rounded-xl" onClick={() => restore(b)}>استرجاع</Button>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {details && (
            <Tabs defaultValue="customer">
              ...
              <TabsContent value="bus" className="space-y-1 text-sm mt-3">
                <Row k="الحافلة" v={details.buses ? (details.buses.name || `حافلة ${details.buses.bus_number}`) : (details.no_bus ? "بدون" : "—")} />
              </TabsContent>
