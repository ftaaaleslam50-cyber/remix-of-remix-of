...
export const Route = createFileRoute('/api/public/push/dispatch')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        if (request.headers.get('x-push-token') !== process.env['PUSH_HOOK_SECRET']) return json({ error: 'Unauthorized' }, 401);
        const payload = await request.json() as { notification_id?: string };
        ...
        for (const subscription of subscriptions) {
          try {
            ...
            if (response.ok) { sent += 1; await supabaseAdmin.from('push_subscriptions').update({ last_error: null, updated_at: new Date().toISOString() }).eq('id', subscription.id); }
            else if (response.status === 404 || response.status === 410) await supabaseAdmin.from('push_subscriptions').update({ is_active: false, last_error: `Push service ${response.status}`, updated_at: new Date().toISOString() }).eq('id', subscription.id);
            else await supabaseAdmin.from('push_subscriptions').update({ last_error: `Push service ${response.status}`, updated_at: new Date().toISOString() }).eq('id', subscription.id);
