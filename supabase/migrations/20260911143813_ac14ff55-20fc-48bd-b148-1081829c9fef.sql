ALTER TABLE public.buses ADD COLUMN IF NOT EXISTS supervisor_name text;

UPDATE public.buses b
SET trip_id = NULL
WHERE b.trip_id = 'f52a2f80-b3b5-459c-bbba-5aac64eb902b'
  AND NOT EXISTS (
    SELECT 1 FROM public.trip_buses tb
    WHERE tb.trip_id = b.trip_id AND tb.bus_id = b.id
  );

CREATE OR REPLACE FUNCTION public.get_ticket(_code text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT to_jsonb(t) FROM (
    SELECT b.booking_code, b.booking_type, b.passenger_count, b.room_type, b.customer_name,
           b.id_number, b.contact_phone, b.whatsapp_phone, b.seat_numbers, b.price_per_person,
           b.total_price, b.discount_amount, b.coupon_code, b.id_image_url, b.created_at,
           b.notes, b.actual_return_day, b.extension_nights, b.trip_mode,
           b.departure_date, b.return_date, b.actual_return_date, b.return_seat_numbers,
           (SELECT jsonb_build_object('name', p.name) FROM public.packages p WHERE p.id = b.package_id) AS packages,
           (SELECT jsonb_build_object('name', h.name) FROM public.hotels h WHERE h.id = b.hotel_id) AS hotels,
           (SELECT jsonb_build_object('name', tr.name, 'departure_day', tr.departure_day, 'return_day', tr.return_day,
                                      'departure_date', tr.departure_date, 'return_date', tr.return_date)
              FROM public.trips tr WHERE tr.id = b.trip_id) AS trips,
           (SELECT jsonb_build_object('bus_number', bu.bus_number, 'name', bu.name, 'plate', bu.plate,
                                      'layout_id', bu.layout_id, 'supervisor_name', bu.supervisor_name)
              FROM public.buses bu WHERE bu.id = b.bus_id) AS buses,
           (SELECT jsonb_build_object('bus_number', rb.bus_number, 'name', rb.name, 'plate', rb.plate,
                                      'supervisor_name', rb.supervisor_name)
              FROM public.buses rb WHERE rb.id = b.return_bus_id) AS return_buses,
           (SELECT bl.layout_json FROM public.buses bu2
              JOIN public.bus_layouts bl ON bl.id = bu2.layout_id
              WHERE bu2.id = b.bus_id) AS layout_json
    FROM public.bookings b
    WHERE b.booking_code = _code AND b.deleted_at IS NULL
    LIMIT 1
  ) t;
$function$;