...
DROP POLICY IF EXISTS "anyone upload id" ON storage.objects;
CREATE POLICY "id uploads own folder" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'id-uploads' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "guest id uploads folder" ON storage.objects FOR INSERT TO anon
  WITH CHECK (bucket_id = 'id-uploads' AND (storage.foldername(name))[1] = 'guest');