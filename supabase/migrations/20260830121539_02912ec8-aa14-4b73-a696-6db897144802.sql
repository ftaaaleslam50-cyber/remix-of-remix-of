ALTER TABLE public.buses ADD COLUMN IF NOT EXISTS open_return_price numeric NOT NULL DEFAULT 0;

ALTER TABLE public.bookings DROP CONSTRAINT IF EXISTS bookings_trip_mode_check;
ALTER TABLE public.bookings ADD CONSTRAINT bookings_trip_mode_check
  CHECK (trip_mode = ANY (ARRAY['round'::text, 'outbound'::text, 'return'::text, 'round_open'::text]));
...
BEGIN
  ...
  IF NEW.trip_mode IS NULL OR NEW.trip_mode NOT IN ('round','outbound','return','round_open') THEN
    NEW.trip_mode := 'round';
  END IF;
  ...
  IF NEW.no_bus IS NOT TRUE AND NEW.bus_id IS NOT NULL THEN
    SELECT CASE NEW.trip_mode
             ...
             WHEN 'round_open' THEN COALESCE(NULLIF(b.open_return_price, 0), NULLIF(b.round_trip_price, 0), b.price_addition, 0)
