ALTER TABLE public.buses
  ADD COLUMN IF NOT EXISTS round_trip_price numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS outbound_price numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS return_price numeric NOT NULL DEFAULT 0;

UPDATE public.buses SET round_trip_price = COALESCE(price_addition, 0) WHERE round_trip_price = 0;

ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS trip_mode text NOT NULL DEFAULT 'round';

DO $$ BEGIN
  ALTER TABLE public.bookings ADD CONSTRAINT bookings_trip_mode_check CHECK (trip_mode IN ('round','outbound','return'));
...
BEGIN
  ...
  IF NEW.trip_mode IS NULL OR NEW.trip_mode NOT IN ('round','outbound','return') THEN
    NEW.trip_mode := 'round';
  END IF;
  IF NEW.trip_mode = 'outbound' THEN
    NEW.actual_return_day := NULL;
  END IF;
  ...
  IF NEW.no_bus IS NOT TRUE AND NEW.bus_id IS NOT NULL THEN
    SELECT CASE NEW.trip_mode
             WHEN 'outbound' THEN COALESCE(NULLIF(b.outbound_price, 0), b.price_addition, 0)
             WHEN 'return' THEN COALESCE(NULLIF(b.return_price, 0), b.price_addition, 0)
             ELSE COALESCE(NULLIF(b.round_trip_price, 0), b.price_addition, 0)
           END
      INTO bus_price
