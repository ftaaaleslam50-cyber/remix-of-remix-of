  ...
  LOOP
    ...
    LOOP
      ...
      IF t.clone_buses_on_advance THEN
        ...
        LOOP
          BEGIN
            UPDATE public.buses SET trip_id = NULL WHERE id = old_bus.id;
      ...
      -- تقديم مواعيد العودة الإضافية (الرحلات ذات أكثر من عودة)
      IF t.return_options IS NOT NULL AND array_length(t.return_options, 1) IS NOT NULL THEN
        SELECT array_agg(
                 CASE
                   WHEN o ~ '^\d{4}-\d{2}-\d{2}$' THEN to_char((o::date + step), 'YYYY-MM-DD')
                   ELSE o
                 END
                 ORDER BY ord
               )
          INTO opts
          FROM unnest(t.return_options) WITH ORDINALITY AS u(o, ord);
        t.return_options := COALESCE(opts, t.return_options);
      END IF;
    ...
    IF guard > 0 THEN
      ...
      SET departure_date = t.departure_date,
          ...
          return_options = t.return_options,
